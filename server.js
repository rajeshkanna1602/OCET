...
{  
 model.findByIdAndUpdate(req.body.id, { 
     cate:req.body.cate, ques:req.body.ques, opt1:req.body.opt1, opt2:req.body.opt2,opt3:req.body.opt3,opt4:req.body.opt4, answer:req.body.answer
    //  name: req.body.name, address: req.body.address, mobno: req.body.mobno
    },  
   function(err,data) {  
   if (err) {  
   res.send(err);         
 ...
 app.post("/api/deleteUser",function(req,res){          
    model.remove({ques: req.body.id}, function(err) {    
     if(err){    
         res.send(err);    
...
app.get('/api/sUser1/:pi',function(req,res){  
 ...
 app.get("/api/getUser1/",function(req,res){  
  ...
  app.get("/api/getquiz1/",function(req,res){  
    model.aggregate([{$sample:{size:15}}],function(err,data){  
              if(err){  
                  res.send(err);  
              }  
              else{                
